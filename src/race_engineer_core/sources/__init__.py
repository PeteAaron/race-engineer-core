...
delegates immediately to the appropriate adapter or runner.
    ...
    DataSource.LIVE_TIMING  — state is built live by LiveTimingClient + ingest()
                              load_state() does not apply; use sources.live_timing directly
...
if TYPE_CHECKING:
    ...
    from ..pipeline.reducer import TimingState
...
def _load_openf1(
    ...
    from .openf1.client import OpenF1Client as _Client
    from .openf1.session import OpenF1SessionLoader
    from ..pipeline.store import EventStore as _Store
